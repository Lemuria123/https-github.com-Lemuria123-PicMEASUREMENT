...
export default function App() {
  ...
  const hoveredMarker = useMemo(() => {
    if (logic.aState.hoveredComponentId) {
      const comp = logic.dState.dxfComponents.find(c => c.id === logic.aState.hoveredComponentId);
      if (comp) {
        // Use the pixel-aware transformer to get normalized coordinates (0-1) from CAD centroid
        const norm = logic.dState.transformer.toNormalizedFromAbsolute(comp.centroid);
        if (norm) {
            return {
              x: norm.x,
              y: norm.y,
    ...
    if (logic.aState.hoveredFeatureId) {
      ...
      if (group) {
        ...
        return { x: normX, y: normY, color: group.color };
      }
    }
