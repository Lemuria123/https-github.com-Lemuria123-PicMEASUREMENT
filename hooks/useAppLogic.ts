...
import { 
  ...
  isPointInPolygon, checkEntityHit, Matrix2D 
...
export function useAppLogic() {
  ...
  useKeyboardNudge({
    currentPoints: interaction.currentPoints,
    setCurrentPoints: interaction.setCurrentPoints,
    getScaleInfo: dState.getScaleInfo,
    imgDimensions: dState.imgDimensions
  });
    ...
    try {
        ...
        if ((mode === 'box_rect' || mode === 'box_poly') && currentPoints.length >= 2) {
            ...
            const isEntityEnclosed = (ent: DxfEntity) => {
                if (ent.minX < polyMinX - ADAPTIVE_EPS || ent.maxX > polyMaxX + ADAPTIVE_EPS || ent.minY < polyMinY - ADAPTIVE_EPS || ent.maxY > polyMaxY + ADAPTIVE_EPS) return false;
            ...
            const isComponentEnclosed = (compId: string): boolean => {
                ...
                return comp.entityIds.every(eid => isEntityEnclosed(dState.dxfEntities.find(e => e.id === eid)!)) && (comp.childGroupIds || []).every(cid => isComponentEnclosed(cid));
            };

            const enclosedGroups = dState.dxfComponents.filter((comp: DxfComponent) => !dState.dxfComponents.some(other => other.id !== comp.id && (other.childGroupIds || []).includes(comp.id)) && isComponentEnclosed(comp.id)).map(c => c.id);
            const handledEntityIds = new Set<string>();
            const collect = (gid: string) => { const c = dState.dxfComponents.find(co => co.id === gid); if(c){ c.entityIds.forEach(eid => handledEntityIds.add(eid)); (c.childGroupIds || []).forEach(collect); } };
            ...
            if (enclosedEntities.length > 0 || enclosedGroups.length > 0) {
                setPromptState({
                  isOpen: true, title: "New Group from Selection", description: `Selected ${enclosedEntities.length} entities and ${enclosedGroups.length} groups.`, defaultValue: `Area Group ${dState.dxfComponents.length + 1}`,
                  onConfirm: (val) => {
                    const preciseGeom = dxfAnalysis.computePreciseGeometry(enclosedEntities, enclosedGroups);
            ...
            } else aState.setMatchStatus({ text: "Selection area empty.", type: 'info' });
            return;
        }
