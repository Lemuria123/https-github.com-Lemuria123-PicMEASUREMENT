...
export function useDxfAnalysis({ dState, aState, setIsProcessing, setMode, setPromptState }: DxfAnalysisProps) {
  ...
  const handleAutoMatch = useCallback(() => {
    ...
    setTimeout(() => {
        const seedEntities = seedGroup.entityIds.map(id => dState.dxfEntities.find(e => e.id === id)).filter(Boolean) as DxfEntity[];
        ...
        const groupW = seedGroup.bounds.maxX - seedGroup.bounds.minX; const groupH = seedGroup.bounds.maxY - seedGroup.bounds.minY;
        ...
        const alreadyGroupedIds = new Set(dState.dxfComponents.flatMap(c => c.entityIds));
        const potentialAnchors = dState.dxfEntities.filter(e => !alreadyGroupedIds.has(e.id) && e.type === s0.type);
        ...
        const propsMatch = (e1: DxfEntity, e2: DxfEntity) => {
            if (e1.type !== e2.type) return false; const T = 0.5;
            if (e1.type === 'CIRCLE') return Math.abs(e1.rawEntity.radius - e2.rawEntity.radius) < T;
            const l1 = Math.sqrt(Math.pow(e1.maxX - e1.minX, 2) + Math.pow(e1.maxY - e1.minY, 2)), l2 = Math.sqrt(Math.pow(e2.maxX - e2.minX, 2) + Math.pow(e2.maxY - e2.minY, 2));
            return Math.abs(l1 - l2) < T;
        };
        potentialAnchors.forEach(candA => {
            if (usedEntityIdsForThisMatchRun.has(candA.id)) return; const ca = getCenter(candA); if (!propsMatch(candA, s0)) return;
            let possibleAngles = [0]; 
            if (seedEntities.length > 1) {
                possibleAngles = [];
                const potentialRefs = dState.dxfEntities.filter(e => !alreadyGroupedIds.has(e.id) && !usedEntityIdsForThisMatchRun.has(e.id) && propsMatch(e, s1));
                potentialRefs.forEach(candR => {
                    const cr = getCenter(candR); const d = Math.sqrt(Math.pow(cr.x - ca.x, 2) + Math.pow(cr.y - ca.y, 2));
                    if (Math.abs(d - refDist) < DYNAMIC_TOLERANCE * 2) { const candAngle = Math.atan2(cr.y - ca.y, cr.x - ca.x); possibleAngles.push(candAngle - refAngle); }
                });
            }
            for (const deltaTheta of possibleAngles) {
                ...
                for (let i = 0; i < seedEntities.length; i++) {
                    if (i === bestAnchorIdx) continue; const s = seedEntities[i]; const cs = getCenter(s); const rotatedOffset = rotate(cs.x - c0.x, cs.y - c0.y, deltaTheta);
                    const targetX = ca.x + rotatedOffset.x, targetY = ca.y + rotatedOffset.y;
                    const found = dState.dxfEntities.find(e => !alreadyGroupedIds.has(e.id) && !tempConsumed.has(e.id) && !usedEntityIdsForThisMatchRun.has(e.id) && propsMatch(e, s) && Math.abs(getCenter(e).x - targetX) < DYNAMIC_TOLERANCE && Math.abs(getCenter(e).y - targetY) < DYNAMIC_TOLERANCE);
                    if (found) { cluster.push(found.id); tempConsumed.add(found.id); minX = Math.min(minX, found.minX); minY = Math.min(minY, found.minY); maxX = Math.max(maxX, found.maxX); maxY = Math.max(maxY, found.maxY); sx += getCenter(found).x; sy += getCenter(found).y; } else { allMatched = false; break; }
                }
                if (allMatched && cluster.length === seedEntities.length) {
                    matchFoundCount++; const matchName = `${seedGroup.name} - Match ${matchFoundCount}`;
                    newMatchGroups.push({ id: generateId(), name: matchName, isVisible: seedGroup.isVisible, isWeld: seedGroup.isWeld, isMark: seedGroup.isMark, color: seedGroup.color, entityIds: cluster, seedSize: seedEntities.length, centroid: { x: sx / cluster.length, y: sy / cluster.length }, bounds: { minX, minY, maxX, maxY }, parentGroupId: seedGroup.id });
                    cluster.forEach(id => usedEntityIdsForThisMatchRun.add(id)); break; 
                }
            }
        });
        if (newMatchGroups.length > 0) { dState.setDxfComponents((prev: DxfComponent[]) => [...prev, ...newMatchGroups]); aState.setMatchStatus({ text: `Auto-Match: Created ${newMatchGroups.length} matching groups!`, type: 'success' }); } 
        else { aState.setMatchStatus({ text: "Auto-Match: No additional matches found.", type: 'info' }); }
        setIsProcessing(false);
    }, 300);
  }, [dState, aState, setIsProcessing]);
